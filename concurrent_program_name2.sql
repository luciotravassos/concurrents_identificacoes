/* Busca o Concurrent pelo nome do concurrent */
SELECT   user_concurrent_program_name, enabled_flag,
   ...
   WHERE queue_control_flag = 'N'
     AND (    UPPER (user_concurrent_program_name) like
                                 upper('%GLDI - Journal Import (Journal Import)%')
        /*  AND (   user_concurrent_program_name LIKE 'pr%'
               OR user_concurrent_program_name LIKE 'pR%'
               OR user_concurrent_program_name LIKE 'Pr%'
               OR user_concurrent_program_name LIKE 'PR%'
              ) */
         )
