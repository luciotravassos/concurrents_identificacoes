/* script will map concurrent manager process information about
--            current concurrent managers */
SELECT   distinct (proc.concurrent_process_id) concproc,
...
--       SUBSTR(proc.logfile_name,1,20) lnam,
    ...
    FROM apps.fnd_concurrent_processes proc,
         ...
         apps.fnd_conc_req_summary_v fsum,
         v$process vproc,
         v$session vsess
   WHERE proc.process_status_code = 'A'
     ...
     AND concq.concurrent_queue_name = 'STANDARD'
